...
CreateSpi::~CreateSpi() { end(); }

bool CreateSpi::begin(spi_host_device_t host_id, int8_t sck, int8_t miso,
                      int8_t mosi, uint32_t frequency) {
...
int CreateSpi::addDevice(spi_device_interface_config_t *device_if_cfg,
                         gpio_num_t cs) {
...
void CreateSpi::csSet(spi_transaction_t *transaction) {
  gpio_set_level(
      static_cast<gpio_num_t>(reinterpret_cast<intptr_t>(transaction->user)),
      1);
}
...
void CreateSpi::pollTransmit(spi_transaction_t *transaction,
                             int device_handle_id) {
  esp_err_t err =
      spi_device_polling_transmit(devices[device_handle_id], transaction);
  if (err != ESP_OK) {
