...
extern "C" void app_main(void) {
    ...
    while (1) {
        ...
        // データの表示
        ESP_LOGI(TAG, "Acceleration (G): X=%.2f Y=%.2f Z=%.2f", data.accel.x, data.accel.y, data.accel.z);
        ESP_LOGI(TAG, "Gyroscope (dps): X=%.2f Y=%.2f Z=%.2f", data.gyro.x, data.gyro.y, data.gyro.z);
