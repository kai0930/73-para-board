...
private:
  ...
  static constexpr int HISTORY_SIZE = 10;   // 保持する履歴データ数
  ...
  std::array<LogData, HISTORY_SIZE> history_buffer;
  static const char *TAG;
  uint32_t start_time;

public:
  LiftoffAccelTest(QueueHandle_t queue)
      : is_detected(false), continuous_count(0), log_queue(queue),
        history_index(0) {
  ...
  void processData(float ax, float ay, float az) {
    ...
    if (accel_buffer_x.size() >= AVERAGE_SAMPLES) {
      ...
      // 履歴データを保存
      history_buffer[history_index] = {
          .timestamp = current_time, .norm2 = norm2, .count = continuous_count};
      ...
      if (norm2 > THRESHOLD_G2) {
        ...
        if (continuous_count >= DETECTION_COUNT && !is_detected) {
          ...
          for (size_t i = 0; i < HISTORY_SIZE; i++) {
            size_t idx = (history_index - HISTORY_SIZE + i) % HISTORY_SIZE;
            xQueueSend(log_queue, &history_buffer[idx], 0);
          }
        }
      } else {
        continuous_count = 0;
  ...
  bool isDetected() const { return is_detected; }
};
