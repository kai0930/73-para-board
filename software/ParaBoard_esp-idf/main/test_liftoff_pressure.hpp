...
class LiftoffPressureTest {
   ...
   public:
    LiftoffPressureTest(QueueHandle_t queue) : is_detected(false), continuous_count(0), log_queue(queue) {
    ...
    void processData(float pressure) {
        ...
        if (pressure_buffer.size() >= AVERAGE_SAMPLES) {
            ...
            // 閾値判定
            if (prev_average_pressure != 0.0f && prev_average_pressure - average_pressure > THRESHOLD_PA) {
                continuous_count++;
            ...
            // ログデータを送信
            LogData log_data = {.timestamp = current_time, .pressure = average_pressure, .count = continuous_count};
